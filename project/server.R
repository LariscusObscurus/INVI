...
distribution <- function(input, output, dataset) {
  ...
  output$peakednessDescription <- renderText({
    ...
    excess <- peakedness - 3
    if (excess < 0) {
      "Negativer Exzess, Flachgipfelig: Die Daten haben wahrscheinlich leichte Ränder."
    } else if (peakedness > 0) {
      "Positiver Exzess, Steilgipfelig: Die Daten haben wahrscheinlich schwere Ränder."
    } else {
      "Exzess = 0 (Kurtosis = 3): Eine perfekte Normalverteilung."
    }
  })
}
